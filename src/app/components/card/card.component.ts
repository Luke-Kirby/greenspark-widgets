...
import { EnvmtData, BadgeText, LoadingState } from 'src/app/core/interfaces/app-interface';
...
export class CardComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dataService.envmtData$.subscribe((data: EnvmtData[]) => this.envmtData = data);
    this.dataService.badgeText$.subscribe((text: BadgeText[]) => this.badgeText = text);
    this.dataService.loadingState$.subscribe((state: LoadingState) => this.loadingState = state);
  ...
  handleBadgeDataEmit(emittedData: EnvmtData): void {
    ...
    // Could move this logic to serive to make it reusable
